...
// Prices a cart — re-validates dish availability, applies the promo code,
// computes the delivery fee — through the exact same code path as
// createOrder(), but writes nothing (create_order()'s p_dry_run). Used by
// the online-payment flow to know how much to charge before an order
// exists at all: the order is only created once payment is confirmed (see
// lib/payments-server.js), so there has to be a way to get the true total
// without a database row to attach it to yet.
export async function priceCart({
  ...
  const { data, error } = await supabase.rpc("create_order", {
    ...
    p_client_id: null,
    p_promo_code: promoCode || null,
    p_payment_method: "card_online",
    p_dry_run: true,
  });
  if (error) throw error;
  return data; // { subtotal, delivery_fee, discount, promo_code, total } — id/order_number are null
}
